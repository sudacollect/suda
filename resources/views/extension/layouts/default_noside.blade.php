...
<body class="suda-body">
    ...
    <nav class="navbar navbar-expand-sm navbar-suda navbar-suda-noside fixed-top navbar-dark bg-dark @if (Auth::guard('operate')->guest()) navbar-nologin @endif">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="nav navbar-nav navbar-custom-navi">
                ...
                <li class="nav-item">
                    <a class="nav-link" href="{{ extadmin_url($navi['url']) }}" target="{{ $navi['target'] }}" title="{{ $navi['name'] }}">@if(isset($navi['icon'])) <i class="{{ $navi['icon'] }}"></i>&nbsp;@endif{{ $navi['name'] }}</a>
                </li>
                @endforeach
            </ul>
            @endif
            ...
            <ul class="nav navbar-nav navbar-operate">
                ...
                @if (Auth::guard('operate')->guest())
                    <li><a href="{{ url('/login') }}">{{ __('suda_lang::press.login') }}</a></li>
                    <li><a href="{{ url('/register') }}">{{ __('suda_lang::press.register') }}</a></li>
                @else
                    
                    
                    @include('view_suda::extension.layouts.operate_top')
                    
                    <!-- support language -->
                    {{-- @include('view_suda::extension.layouts.top_language') --}}
