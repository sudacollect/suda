...
@foreach ($items as $item_key=>$item)
    ...
    <div class="panel" id="panel-{{ $loop->iteration }}" @if($current_item) x-init="show_current_menu = {{ $loop->iteration }}" @endif>
        <div @click="toggle({{ $loop->iteration }})" class="panel-heading @if(!$hasChildren) only-link @endif collapse show @if(!isset($current_item) || !$current_item) collapsed @endif" @if($hasChildren) data-bs-toggle="collapse" data-bs-target="#sidebar-menu-{{ $item->slug }}" @endif aria-controls=".sidebar-menu-{{ $item->slug }}" aria-expanded="true">
        ...
        @include('view_suda::admin.livewire.menu.sidebar_children',['iteration'=>$loop->iteration,'root_group'=>$root_group])
