...
<tr data_child = "{{ $child }}" @if(isset($toggle) && $toggle==1) style="display:none" @endif>
  ...
  <td @if($child>0) style="border-top:none" @endif>
      ...
      <a href="{{ $buttons['create'].'/'.$cate->id }}" class="pop-modal btn btn-light btn-xs" data-modal-id="add-taxonomy-category" title="{{ __('suda_lang::press.btn.add') }}"><i class="ion-add"></i></a>
      @endif
  ...
  <td @if($child>0) style="border-top:none" @endif >
      <a href="{{ $buttons['update'].'/'.$cate->id }}" class="pop-modal btn btn-light btn-xs" title="{{ __('suda_lang::press.edit') }}"><i class="ion-create"></i>&nbsp;{{ __('suda_lang::press.edit') }}</a>
      @if($cate->term->slug!='default')
        <button href="{{ $buttons['delete'].'/'.$cate->id }}" class="pop-modal-delete btn btn-light btn-xs" data_id="{{ $cate->id }}" title="{{ __('suda_lang::press.delete') }}">
            <i class="ion-trash"></i>
        </button>
      @endif
  </td>
