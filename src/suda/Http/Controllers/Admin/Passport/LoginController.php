...
class LoginController extends AdminController
{
    ...
    public function showLoginForm(SessionStore $sessionStore,Request $request) {
        ...
        if(array_key_exists('dashboard',$this->settings)){
            if(array_key_exists('loginbox',$this->settings['dashboard'])){
                //$login_view = $this->settings['dashboard']['loginbox']=='picture'?'login-split-image':'login';
                if(in_array($this->settings['dashboard']['loginbox'],['simple-blue','simple-orange','simple-cyan','simple-black'])){
                    $login_view = 'login-simple';
                    $this->setData('simpleName',$this->settings['dashboard']['loginbox']);
                }
    ...
    public function login(SessionStore $sessionStore,Request $request)
    {
        ...
        if ($this->attemptLogin($request)) {
            ...
            if(auth('operate')->user()->user_role>0)
            {
                //设置跳转入口
                if(auth('operate')->user()->user_role==2){
                    $this->redirectTo = admin_url('entry/extensions');
                }
