        ...
        foreach ($request->file as $one) {
            if ($this->allowUpload($one)) {
                ...
                try {
                    ...
                    //保存至Media数据库
                    $mediaObj = new Media;
                    $mediaObj->name = $name_only;
                    $mediaObj->type = in_array($ext_only,['jpeg','bmp','png','jpg','gif','svg'])?strtoupper($ext_only):'FILE|'.$ext_only;
                    $mediaObj->user_type = 'operate';
                    $mediaObj->user_id = $this->user->id;
                    $mediaObj->size = $one->getSize();
                    $mediaObj->path = 'public/'.$full_path;
                    $mediaObj->save();
